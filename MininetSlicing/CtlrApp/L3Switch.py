...
class L3Switch(app_manager.RyuApp):
    ...
    def _packet_in_handler(self, ev):
        ...
        if dst in self.IP_to_port[dpid]:
            ...
            self.logger.info("Pkt={dpid=%s,inport=%s,outport=%s,src=%s,dst=%s}",dpid,in_port,out_port,src,dst)
        else:
            out_port = ofproto.OFPP_FLOOD
            self.logger.info("[MatchNotFound]: CtlrFlowTable="+json.dumps(self.IP_to_port))

        actions = [parser.OFPActionOutput(out_port)]

        # install a flow to avoid packet_in next time
        if out_port != ofproto.OFPP_FLOOD:
            match = parser.OFPMatch(in_port=in_port, ipv4_src=src,ipv4_dst=dst)
